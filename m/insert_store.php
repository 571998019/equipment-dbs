...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form-store")) {
  $insertSQL = sprintf("INSERT INTO tb_stores (stores_name) VALUES (%s)",
                       GetSQLValueString($_POST['name'], "text"));

  mysql_select_db($database_connectioneq, $connectioneq);
  $Result1 = mysql_query($insertSQL, $connectioneq) or die(mysql_error());

  $insertGoTo = "detail-store.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<p style="text-align:center;"><b>เพิ่มอุปกรณ์</b></p>
  <form action="<?php echo $editFormAction; ?>" method="POST" name="form-store">
    <table align="center">
  	<tr valign="baseline">
      <td nowrap="nowrap" align="right">Store Name :</td>
      <td><input type="text" id="name" name="name" placeholder="ชื่อสถานที่เก็บอุปกรณ์.."></td>
    </tr>
    <tr valign="baseline">
      ...
      <td><input type="submit" class="btn btn-success" value="ยืนยัน">
    ...
    <input type="hidden" name="MM_insert" value="form-store" />
  </form>
</div>
</div>
